...
public class VentaServiceImpl implements VentaService {
    ...
    public VentaDTO registrarVenta(VentaDTO ventaDTO) {
        ...
        Usuario usuario = usuarioRepository.findById(ventaDTO.getUsuarioId())
                .orElseThrow(() -> new RuntimeException("Usuario no encontrado ID: " + ventaDTO.getUsuarioId()));

        SesionCaja sesion = sesionCajaRepository.findById(ventaDTO.getSesionId())
                .orElseThrow(() -> new RuntimeException("Sesion no encontrada ID: " + ventaDTO.getSesionId()));

        // (Optional: Check if sesion is open, etc.)
        ...
        for (DetalleVentaDTO detDTO : ventaDTO.getDetalles()) {
            ...
            if (producto.getCantidadStock() < detDTO.getCantidad()) {
                throw new StockInsuficienteException("Stock insuficiente para producto: " + producto.getNombre() +
                        ". Disponible: " + producto.getCantidadStock() + ", Requerido: " + detDTO.getCantidad());
            }
